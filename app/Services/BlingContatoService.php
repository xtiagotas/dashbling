<?php

namespace App\Services;

use App\Models\BlingToken;
use App\Models\Contato;
use App\Services\BlingDataService;

class BlingContatoService
{
    private $bling_token;
    private $blingDataService;

    public function __construct(BlingToken $bling_token)
    {
        $this->bling_token = $bling_token;
        $this->blingDataService = new BlingDataService($bling_token);
    }

    public function execute()
    {
        $page = 1;
        $continue = true;

        do {
            $url = '/contatos?pagina=' . $page. '&limite=100';
            $result = $this->blingDataService->execute($url);

            if (array_key_exists('data', $result) && (count($result['data']) > 0)) {

                foreach ($result['data'] as $data) {

                    if (Contato::where('bling_id', $data['id'])->count() > 0) {

                        $continue = false;
                    } else {

                        $contato = new Contato();
                        $contato->user_id = $this->bling_token->user_id;
                        $contato->bling_id = $data['id'];
                        $contato->nome  = $data['nome'];
                        $contato->codigo  = $data['codigo'];
                        $contato->situacao  = $data['situacao'];
                        $contato->fantasia  = '';
                        $contato->tipo  = '';
                        $contato->endereco_uf  = '';
                        $contato->endereco_municipio  = '';
                        $contato->sync = '';  
                        $contato->save();  

                        // SyncBlingContatoDetalhe::dispatch($contato);
                    }
                }

                $page++;
            } else {
                
                $continue = false;
            }
        } while ($continue);

        return;
    }
}
