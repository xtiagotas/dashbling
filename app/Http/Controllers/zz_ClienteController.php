...
class ClienteController extends Controller
{
    public function index(Request $request)
    {
        ...
        for ($i = 0; $i < count($clientes); $i++) {
            ...
            $clientes[$i]->dias_ultima_venda = (new DateTime($ultimo_pedido->dataSaida))->diff($dataAtual)->days;
    ...
    public function show(Request $request)
    {
        ...
        $contato_id = $request['cliente'];

        $contato = Contato::where('user_id', $user_id)->where('bling_id', $contato_id)->firstOrFail();
