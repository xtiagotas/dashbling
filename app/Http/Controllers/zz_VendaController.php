...
class VendaController extends Controller
{
    public function index(Request $request)
    {
        ...
        // $dataAtual = new DateTime('2021-09-28');

        $dataMenos30D = new DateTime($dataAtual->format('Y-m-d'));
        ...
        $periodo = $request['periodo'] ?? $dataMenos30D->format('Y-m-d') . ' - ' . $dataAtual->format('Y-m-d');

        $situacoes_filter = $user->situacoes->where('modulo_id', '=', '98310');
        $lojas_filter = $user->lojas;

        $periodo_de = new DateTime(implode('-', array_reverse(explode('/', substr($periodo, 0, 10)))));
        $periodo_ate = new DateTime(implode('-', array_reverse(explode('/', substr($periodo, 13, 10)))));
        ...
        $pedidos = Pedido::where('user_id', $user->id)
            ...
            ->where('dataSaida', '>=', $periodo_de->format('Y-m-d'))
            ->where('dataSaida', '<=',  $periodo_ate->format('Y-m-d'))
            ->get();
        ...
        $pedidos_dias =  DB::table('pedidos')
            ...
            ->where('situacao_id', '=', $situacao_id)
            ->where('pedidos.dataSaida', '>=', $periodo_de->format('Y-m-d'))
            ->where('pedidos.dataSaida', '<=',  $periodo_ate->format('Y-m-d'))
            ->groupBy('dataSaida')
            ->orderBy('data')
            ->get();
        ...
        return view('vendas.index',
            [
                ...
                'periodo_de' => str_replace('-', '/', $periodo_de->format('Y-m-d')),
                'periodo_ate' => str_replace('-', '/', $periodo_ate->format('Y-m-d')),
